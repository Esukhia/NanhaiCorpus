...
def total_vocabulary(files_list):
    total = []
    for f in files_list:
...
def compare_lists(base_list, to_compare_list):
    base = total_vocabulary(base_list)
    to_compare = total_vocabulary(to_compare_list)
...
# #
# tsikchen = ['../pytib/user_vocabs/tsikchen.txt']
# corpus_files = ['../pytib/user_vocabs/oral_corpus_{}.txt'.format(a) for a in range(1, 4)] + ['../pytib/user_vocabs/recordings_4.txt']
# no_tsikchen, in_tsikchen = compare_lists(tsikchen, corpus_files)
# write_file('no_tsikchen.txt', '\n'.join(tib_sort(no_tsikchen)))
